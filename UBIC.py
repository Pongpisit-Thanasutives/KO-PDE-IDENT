...
def AIC(prediction, ground, nparams):
    llf = log_like_value(prediction, ground)
    return -2*llf + 2*nparams

def BIC(prediction, ground, nparams):
    llf = log_like_value(prediction, ground)
...
# For fitted LinearRegression & single y: sklearn.metrics.make_scorer(BIC_score, estimator=estimator). Not for multi-task regression
